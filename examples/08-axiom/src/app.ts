/**
 * Hono application with logscope + Axiom sink.
 *
 * Logs go to two sinks in parallel:
 *   1. Console — colorful ANSI output in the terminal (immediate)
 *   2. Axiom  — batched export to the mock Axiom ingest endpoint
 *
 * The Axiom sink uses createAxiomSink() which internally batches records,
...
// ============================================================================
// 1. Configure logscope with console + Axiom sinks
// ============================================================================

const MOCK_AXIOM_PORT = 3106
const MOCK_AXIOM_URL = `http://localhost:${MOCK_AXIOM_PORT}`

const log = createLogger('my-app')

// Create the Axiom sink pointed at our mock server
const axiomSink = createAxiomSink({
  dataset: 'example-logs',
  token: 'xaat-example-token-for-demo',
  url: MOCK_AXIOM_URL,
  batch: {
  ...
  onDropped: (batch, error) => {
    console.log(
      `\x1b[31m  [axiom-sink] Dropped ${batch.length} records: ${error}\x1b[39m`,
    )
  },
})
...
await configure({
  ...
  loggers: [
    { category: 'my-app', level: 'debug', sinks: ['console', 'axiom'] },
  ],
...
// --- GET / — baseline route ---
app.get('/', (c) => {
  return c.json({ message: 'Welcome to the logscope Axiom example!' })
})
...
app.get('/flood', (c) => {
  ...
  return c.json({
    message: 'Sent 50 log messages. Watch the terminal for onDropped callbacks and batched Axiom events.',
  })
})
...
export function startApp(): Promise<void> {
  return new Promise((resolve) => {
    serve({ fetch: app.fetch, port: APP_PORT }, () => {
      console.log(`\n  Hono + logscope + Axiom example on http://localhost:${APP_PORT}\n`)
...
// Graceful shutdown — flushes remaining Axiom batches
export async function shutdown(): Promise<void> {
  console.log('\nShutting down — flushing remaining Axiom batches...')
  await dispose()
  process.exit(0)
}
